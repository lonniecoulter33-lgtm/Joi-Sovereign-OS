...
if sys.version_info < (3, 11):
    ...
    raise RuntimeError("Joi requires Python 3.11 or newer.")
...
MODULES:
  ...
  joi_filesystem.py  - File read/write/search tools
  joi_web.py         - Web search & fetch
  joi_avatar.py      - Avatar generation, TTS
  joi_files.py       - PDF/DOCX/TXT/MD generation
...
def _build_context_parallel(user_message: str, recent: list) -> tuple:
    """
    ...
    tasks = [
        ...
        (14, "WORKING_MEMORY", lambda: (
            __import__("modules.joi_memgpt", fromlist=["compile_working_memory"]).compile_working_memory(user_message) or "",
            "WORKING_MEMORY",
        )),
...
def chat():
    ...
    # Check for mute/unmute text triggers (skip LLM entirely)
    if user_message:
        try:
            from modules.joi_commentary import check_mute_trigger
    ...
    if user_message and any(p in user_message.lower() for p in _TOOL_QUERY_PATTERNS):
        try:
            ...
            system_content += (
                f"\n\n[CAPABILITY QUERY ACTIVE -- {USER_NAME} IS ASKING ABOUT YOUR TOOLS]:\n"
                f"{_tool_block}\n"
                f"MANDATORY: READ the list above right now and present it to {USER_NAME}. "
                f"Say: 'I have [N] tools. Here they are:' then list ALL tools grouped by category. "
                f"Do NOT call any tool to do this -- the full list is already above. "
                f"Do NOT say 'I don't have tools'. Do NOT list only 4 examples. "
                f"Present EVERY tool in the list above. All {len([l for l in _tool_block.split(chr(10)) if l.strip().startswith('-')])} categories.\n"
            )
    ...
    if _user_asked_tools and (any(p in reply.lower() for p in _TOOL_DENIAL_PHRASES) or _shallow_tool_reply):
        try:
            ...
            for dom, info in sorted(domains.items()):
                tools_in_dom = info.get("tools", [])
                if tools_in_dom:
                    label = dom.replace("_", " ").title()
                    lines.append(f"**{label}**: " + ", ".join(f"`{t}`" for t in tools_in_dom))
            tools_text = "\n".join(lines)
            reply = (
                f"ok actually let me give you the real list. i have **{total} tools** registered right now:\n\n"
                f"{tools_text}\n\n"
    ...
    result = {"ok": True, "reply": reply, "model": model_used}
