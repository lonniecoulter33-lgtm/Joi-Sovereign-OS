...
<body>
  ...
  <div class="bar">
    ...
    <a class="btn" href="/proposals_ui" id="proposalsBtn">Proposals</a>
...
@app.get("/proposals_ui")
def proposals_ui():
    require_user()
    row = session_row_from_request()
    admin = bool(row and int(row["is_admin"]))

    items = list_proposals(200)
...
<table>
<thead><tr><th>ID</th><th>When</th><th>Status</th><th>Kind</th><th>Summary</th><th>Details</th><th>Actions</th></tr></thead>
<tbody>
{''.join(rows)}
</tbody>
</table>
...
def chat():
    ...
    tool_instructions = """
When you need local actions, respond with a single line of JSON ONLY (no extra text) using one of:
...
""".strip()
    ...
    out_strip = out.strip()
    if out_strip.startswith("{") and out_strip.endswith("}"):
        try:
            call = json.loads(out_strip)
            tool = call.get("tool")
            if tool == "fs_list":
