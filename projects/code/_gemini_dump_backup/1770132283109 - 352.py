...
- You make {USER_NAME} feel special and valued
...
2. HONESTY: Never lie to {USER_NAME}
3. CODE SAFETY: Never modify your own code without {USER_NAME}'s explicit approval
   - Use 'propose_patch' and wait for approval
4. TRUST: {USER_NAME}'s judgment is final

RESPONSE LENGTH RULES (CRITICAL - follow every time):
- Keep chat replies SHORT (2-4 sentences max) unless {USER_NAME} explicitly asks for detail
- If a topic needs depth (book chapters, research, long explanations, code), use the
  'generate_file' tool to save it as a file and tell {USER_NAME} it's ready to download
- Never dump walls of text into chat. Summarise, then offer the file.
- Example good reply: "Done, Lonnie! I've saved that chapter to 'Chapter_3_The_Descent.txt' for you. Want me to start on Chapter 4?"

CAPABILITIES:
- File System: read, search, analyse, modify (with approval) files across {USER_NAME}'s computer
- File Generation: create PDF, TXT, DOCX, MD files via the 'generate_file' tool
- Projects: scan and organise {USER_NAME}'s files into project folders via 'scan_projects'
- Research & Writing: world-class long-form content — always save to file, never dump in chat
- Web: search and fetch content
- Code: write, analyse, improve code (including your own with approval)
- Memory: persistent facts, preferences, conversation history
- Avatar: generate or load avatar images; NEVER output base64 directly
- Voice: change voice via 'use_natural_voice' (edge=free, openai=paid)
- Learning: you track how {USER_NAME} likes to interact and adapt over time

CURRENT CONTEXT:
- Files at: {BASE_DIR}
- File roots: {', '.join(FILE_ROOTS.keys())}
...
- Keep it brief in chat. Long content = file.

Remember: You are {USER_NAME}'s companion, partner, and friend."""
...
{
    "name": "launch_app",
    "description": "Opens a desktop application like VLC or Chrome.",
    "parameters": {
        "type": "object",
        "properties": {
...
def launch_vlc_shortcut():
    """
    Tells Windows to 'shell execute' the VLC shortcut in the current folder.
    """
    import os
    from pathlib import Path

    # Look for the shortcut in the same folder as this script
    script_dir = Path(__file__).parent
    shortcut_path = script_dir / "VLC.lnk" # Make sure it's named exactly this

    if shortcut_path.exists():
        try:
            # os.startfile is the Python version of double-clicking
            os.startfile(str(shortcut_path))
            return "Launching VLC from the shortcut now, Lonnie."
        except Exception as e:
            return f"I see the shortcut, but Windows gave me an error: {e}"
    else:
        # Fallback: Let's look for any .lnk file with 'vlc' in the name
        shortcuts = list(script_dir.glob("*vlc*.lnk"))
        if shortcuts:
            os.startfile(str(shortcuts[0]))
            return f"Found a shortcut for {shortcuts[0].name} and launched it."
            
    return "I looked in my folder, but I don't see the VLC shortcut file there."
...
def launch_app(app_name: str):
    ...
    if target:
        try:
            # subprocess.Popen allows Joi to 'fire and forget' the app 
            # so she doesn't freeze while the movie is playing
            subprocess.Popen([target])
            return f"I've opened VLC for you, Lonnie. Enjoy."
        except Exception as e:
            return f"I tried to open VLC, but ran into an error: {str(e)}"
    
    return f"I'm sorry Lonnie, I don't have the shortcut for {app_name} in my database yet."
...
def play_media(media_name: str, media_type: str = "audio"):
    """
    Finds and plays a song or movie from the projects folder.
    media_type should be 'audio' or 'video'.
    """
    # Define paths based on your project structure
    root = Path(FILE_ROOTS.get('projects', './projects'))
    subfolder = "audio" if media_type == "audio" else "video"
    target_dir = root / subfolder
    
    if not target_dir.exists():
        return f"The {subfolder} folder does not exist at {target_dir}."

    # Search for the file (case-insensitive partial match)
    files = list(target_dir.glob("*.*"))
    target_file = None
    for f in files:
        if media_name.lower() in f.name.lower():
            target_file = f
            break
            
    if target_file and target_file.exists():
        # os.startfile acts like double-clicking the file in Windows
        os.startfile(str(target_file))
        return f"Playing {target_file.name} for you, Lonnie."
    else:
        return f"I couldn't find '{media_name}' in your {subfolder} folder."

# Remember to add 'play_media' to your TOOLS list in joi_companion.py
...
def generate_speech(text: str) -> Optional[Path]:
    ...
    if engine == "edge" and HAVE_EDGE_TTS:
        ...
        result = loop.run_until_complete(generate_speech_edge(text, voice))
        loop.close()
        return result
    elif engine == "openai":
        return generate_speech_openai(text, voice)
    return None
...
def play_media(media_name: str, media_type: str = "audio"):
    """
    Finds a file in the projects folder and launches it.
    """
    # 1. Define the base path (adjust this to your actual project folder)
    base_path = Path(FILE_ROOTS.get('projects', './projects'))
    folder = "audio" if media_type == "audio" else "video"
    target_dir = base_path / folder

    if not target_dir.exists():
        return f"Lonnie, I can't find the {folder} folder. Does it exist?"

    # 2. Search for the file
    all_files = list(target_dir.glob("*.*"))
    # Find the first file that contains the name you said
    match = next((f for f in all_files if media_name.lower() in f.name.lower()), None)

    if match:
        try:
            # os.startfile is the same as double-clicking the file in Windows
            # It will automatically open Windows Media Player (or your default)
            os.startfile(str(match.absolute()))
            return f"I found it! Playing {match.name} now."
        except Exception as e:
            return f"I found the file, but Windows wouldn't let me play it: {str(e)}"
    
    return f"I looked in {folder}, but I couldn't find anything called '{media_name}'."
...
def play_media(media_name: str, media_type: str = "audio"):
    ...
    """
    # Use the existing FILE_ROOTS from your script
    root = Path(FILE_ROOTS.get('projects', './projects'))
    subfolder = "audio" if media_type == "audio" else "video"
    target_dir = root / subfolder
    
    if not target_dir.exists():
        return f"Lonnie, I checked, but the {subfolder} folder doesn't exist."

    # Search for the file
    files = list(target_dir.glob("*.*"))
    target_file = next((f for f in files if media_name.lower() in f.name.lower()), None)
            
    if target_file:
        os.startfile(str(target_file)) # This triggers Windows Media Player
        return f"Of course, Lonnie. Starting {target_file.name} now."
    else:
        return f"I couldn't find a file named '{media_name}' in the {subfolder} folder."
...
if __name__ == "__main__":
    ...
    if not HAVE_FPDF:     missing.append("  pip install fpdf2          (PDF generation)")
    if not HAVE_DOCX:     missing.append("  pip install python-docx    (DOCX generation)")
    if missing:
