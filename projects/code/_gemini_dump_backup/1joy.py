...
# Context/limits to reduce token spikes (helps avoid 429 TPM errors)
RECENT_MSG_LIMIT = int(os.getenv("JOI_RECENT_MSG_LIMIT", "8"))
MAX_CHARS_PER_MESSAGE = int(os.getenv("JOI_MAX_CHARS_PER_MESSAGE", "2000"))
MAX_TOTAL_CONTEXT_CHARS = int(os.getenv("JOI_MAX_TOTAL_CONTEXT_CHARS", "14000"))
MAX_OUTPUT_TOKENS = int(os.getenv("JOI_MAX_OUTPUT_TOKENS", "1200"))
...
FILE_ROOTS = {
    ...
    "workspace": os.getenv(
        "JOI_ROOT_WORKSPACE",
        _default_root(r"C:\AI_WORKSPACE"),
    ),

    "downloads": os.getenv(
        "JOI_ROOT_DOWNLOADS",
        _default_root(os.path.join(USERPROFILE, "Downloads")),
    ),

    "documents": os.getenv(
        "JOI_ROOT_DOCUMENTS",
        _default_root(os.path.join(USERPROFILE, "Documents")),
    ),

    "desktop": os.getenv(
        "JOI_ROOT_DESKTOP",
        _default_root(os.path.join(USERPROFILE, "Desktop")),
    ),

    "pictures": os.getenv(
        "JOI_ROOT_PICTURES",
        _default_root(os.path.join(USERPROFILE, "Pictures")),
    ),

    # common OneDrive path
    "onedrive_documents": os.getenv(
        "JOI_ROOT_ONEDRIVE_DOCS",
        _default_root(os.path.join(USERPROFILE, "OneDrive", "Documents")),
    ),

    "onedrive_pictures": os.getenv(
        "JOI_ROOT_ONEDRIVE_PICS",
        _default_root(os.path.join(USERPROFILE, "OneDrive", "Pictures")),
    ),
}
...
def recent_messages(limit: int = 20) -> List[Dict[str, str]]:
    """
    Load recent chat messages from SQLite, but apply conservative truncation.
    This prevents token-per-minute spikes (e.g., when older messages contain big dumps).
    """
    ...
    for r in rows:
        ...
        # Enforce total context cap (keep the most recent messages)
        if MAX_TOTAL_CONTEXT_CHARS > 0 and (total_chars + len(content)) > MAX_TOTAL_CONTEXT_CHARS:
            # stop adding older context once we hit the cap
            continue
...
async function openProposal(){
  const pid = prompt("Proposal ID to open (number):");
  if(pid===null) return;
  const n = pid.trim();
  if(!n) return;
  // Open a friendly HTML view in a new tab (keeps cookies)
  window.open(`/proposal_view/${encodeURIComponent(n)}`, "_blank");
}

async function approveProposal(){
  const pid = prompt("Proposal ID to approve (number):");
  if(pid===null) return;
  const n = pid.trim();
  if(!n) return;
  const res = await fetch(`/proposal/${encodeURIComponent(n)}/approve`, {method:"POST"});
  const data = await res.json();
  addLine("Joi", data.ok ? `Approved #${n}.` : (`Approve failed: ${data.error || res.status}`), data.ok ? "ai":"err");
}

async function rejectProposal(){
  const pid = prompt("Proposal ID to reject (number):");
  if(pid===null) return;
  const n = pid.trim();
  if(!n) return;
  const res = await fetch(`/proposal/${encodeURIComponent(n)}/reject`, {method:"POST"});
  const data = await res.json();
  addLine("Joi", data.ok ? `Rejected #${n}.` : (`Reject failed: ${data.error || res.status}`), data.ok ? "ai":"err");
}

async function applyProposal(){
  const pid = prompt("Proposal ID to apply (number):");
  if(pid===null) return;
  const n = pid.trim();
  if(!n) return;
  const res = await fetch(`/proposal/${encodeURIComponent(n)}/apply`, {method:"POST"});
  const data = await res.json();
  if(data.ok){
    const msg = data.diff ? (`Applied #${n}.\n\nDiff:\n` + data.diff) : `Applied #${n}.`;
    addLine("Joi", msg, "ai");
  } else {
    addLine("Joi", `Apply failed: ${data.error || res.status}`, "err");
  }
}
...
@app.get("/proposal_view/<int:pid>")
def proposal_view(pid: int):
    require_user()
    row = get_proposal(pid)
    if not row:
        return "<h3>Proposal not found.</h3>", 404
    # Render a simple page that keeps cookies and offers action buttons.
    status = row["status"]
    summary = row["summary"]
    ts = row["ts"]
    kind = row["kind"]
    ...
    <head>
      ...
      <style>
        body {{ font-family: Arial, sans-serif; margin: 22px; }}
        pre {{ white-space: pre-wrap; background:#fafafa; border:1px solid #ddd; padding:12px; border-radius:12px; }}
        .bar {{ display:flex; gap:10px; flex-wrap:wrap; align-items:center; margin: 12px 0; }}
        button {{ padding:8px 12px; }}
        .chip {{ padding:6px 10px; border:1px solid #ddd; border-radius:999px; background:#fff; }}
      </style>
    </head>
    <body>
      <h2>Proposal #{pid}</h2>
      ...
      <script>
        async function act(action){{
          const res = await fetch(`/proposal/{pid}/${{action}}`, {{method:"POST"}});
          const data = await res.json();
          document.getElementById("out").textContent = JSON.stringify(data, null, 2);
        }}
      </script>
...
def parse_meta_command(text: str) -> Optional[str]:
    ...
    if m:
        ...
        return f"To {action} proposal #{pid}, use the Admin Login button, then click the {action.title()} proposal button (or POST to /proposal/{pid}/{action})."
...
def chat():
    ...
    # Recent messages
    msgs.extend(recent_messages(RECENT_MSG_LIMIT))
...
""".strip()
    ...
    # Call model
    try:
        resp = client.responses.create(model=MAIN_MODEL, input=msgs, tools=tools if tools else None, max_output_tokens=MAX_OUTPUT_TOKENS)
        out = getattr(resp, "output_text", "") or ""
    except Exception as e:
        # OpenAI SDK raises RateLimitError for 429; but keep a string fallback.
        name = type(e).__name__
        msg = str(e)
        if "RateLimitError" in name or "rate_limit" in msg.lower() or "Error code: 429" in msg:
            return jsonify({"ok": False, "reply": f"Rate limit hit (429). Please wait a few seconds and try again. Details: {name}: {msg}"}), 429
        return jsonify({"ok": False, "reply": f"Server error: {name}: {msg}"}), 500
