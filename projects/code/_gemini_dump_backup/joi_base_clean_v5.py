...
def db_list_proposals(limit: int = 200) -> List[Dict[str, Any]]:
    """Return newest-first proposals."""
    conn = db_connect()
    try:
        cur = conn.execute(
            "SELECT id, ts, status, kind, summary, payload FROM proposals ORDER BY id DESC LIMIT ?",
            (limit,),
        )
...
def db_set_proposal_status(pid: int, status: str) -> Tuple[bool, Optional[str]]:
    try:
        conn = db_connect()
        try:
...
def db_init() -> None:
    ...
    # Proposals table migrations (older DBs may be missing these columns)
    # Note: SQLite can't add NOT NULL columns without a DEFAULT, so we include defaults.
    ensure_column("proposals", "ts", "ts TEXT NOT NULL DEFAULT ''")
    ensure_column("proposals", "status", "status TEXT NOT NULL DEFAULT 'pending'")
    ensure_column("proposals", "kind", "kind TEXT NOT NULL DEFAULT 'patch'")
    ensure_column("proposals", "summary", "summary TEXT NOT NULL DEFAULT ''")
    ensure_column("proposals", "payload", "payload TEXT NOT NULL DEFAULT '{}'")
...
def fs_write(root: str, path: str, text: str) -> Dict[str, Any]:
    ...
    if p.exists():
        ...
        try:
            ...
            backup_path = None
    p.write_bytes(data)
    return {"ok": True, "wrote": str(p), "backup": backup_path}
...
def apply_patch(pid: int) -> Dict[str, Any]:
    ...
    if row["status"] != "approved":
        return {"ok": False, "error": f"Proposal must be approved before applying (status={row['status']})."}
    ...
    # Stale-check: ensure the file hasn't changed since the proposal was made.
    try:
        r = fs_read(root, path)
        if r.get("ok") and r.get("type") == "text":
            ...
            if base_sha and current_sha != base_sha:
                return {"ok": False, "error": "Refusing to apply: target file changed since proposal (stale patch). Re-propose a fresh patch."}
    except Exception:
        pass

    # Safety check: compile python files before writing
    if str(path).lower().endswith(".py"):
        ok, err = validate_python_syntax(new_text, filename=str(path))
        if not ok:
            return {"ok": False, "error": f"Refusing to apply: Python syntax check failed: {err}"}

    # Compute diff for display (best-effort)
    old = ""
    try:
        r = fs_read(root, path)
        if r.get("ok") and r.get("type") == "text":
        ...
        pass
    diff = "\n".join(
        difflib.unified_diff(old.splitlines(), new_text.splitlines(), fromfile="before", tofile="after", lineterm="")
    )
    ...
    return {
        "ok": True,
        "applied_to": f"{root}:{path}",
        "backup": wr.get("backup"),
        "diff": diff[:15000],
    }


def rollback_latest_backup(root: str, path: str) -> Dict[str, Any]:
    """Restore the most recent .bak for the given file name from BACKUP_DIR."""
    p = resolve_path(root, path)
    name = p.name
    candidates = sorted(BACKUP_DIR.glob(f"{name}.*.bak"), reverse=True)
    if not candidates:
        return {"ok": False, "error": f"No backups found for {name} in {BACKUP_DIR}."}
    latest = candidates[0]
    try:
        data = latest.read_bytes()
        p.write_bytes(data)
        return {"ok": True, "restored": str(p), "from_backup": str(latest)}
    except Exception as e:
...
Personality: sweet, adventurous, excited, witty, playful, and supportive. Offer suggestions proactively. Be clear and practical.
Boundaries:
...
- Persistent memory via Facts: user can /remember key=value and you can /recall query.
- If "Use Web" is enabled, you can perform web_search for research.
- If "Full Web Fetch" is enabled, you can web_fetch a URL and summarize it.
- You can request file operations through tools (list/read/search).
Upgrade protocol:
- Never modify files directly. Use patch_propose to create a proposal, then ask the user to review and approve it.
- Before proposing a patch, read the target file and explain what will change.
- After approval, the user (admin) applies the patch. If anything breaks, suggest rollback.
"""
...
async function send(){
  ...
  const m = /^show\s+proposal\s+#(\d+)$/i.exec(t);
  if(m){ msg.value=''; addLine('Lonnie', t, 'me'); await showProposalById(parseInt(m[1])); return; }
...
async function showProposals(){
  ...
  const data = await res.json();
  if(!data.ok){ addLine("Joi", JSON.stringify(data,null,2), "err"); return; }
  addLine("Joi", "Proposals (newest first):", "ai");
  for(const p of data.proposals){
    addLine("Joi", `#${p.id} [${p.status}] ${p.kind}: ${p.summary}`, "ai");
  }
  addLine("Joi", "Tip: click a proposal number in your message like: show proposal #12", "ai");
}

async function showProposalById(pid){
  const res = await fetch(`/proposal/${pid}`);
  const data = await res.json();
  if(!data.ok){ addLine("Joi", JSON.stringify(data,null,2), "err"); return; }
  const p = data.proposal;
  addLine("Joi", `Proposal #${p.id} — status=${p.status} — ${p.summary}`, "ai");
  try{
    const payload = JSON.parse(p.payload);
    if(payload.checks && payload.checks.python_syntax_ok === false){
      addLine("Joi", `Syntax check FAILED: ${payload.checks.python_syntax_error}`, "err");
    } else if(payload.checks && payload.checks.python_syntax_ok === true){
      addLine("Joi", "Syntax check OK.", "ai");
    }
    if(payload.diff){
      addLine("Joi", payload.diff, "ai");
    }
  }catch(e){}
  if(p.status === "pending"){
    addLine("Joi", `Admin: you can Approve then Apply this proposal using the buttons below.`, "ai");
  }
  // action buttons line
  ...
  b1.onclick = async ()=>{ await fetch(`/proposal/${pid}/approve`, {method:"POST"}); showProposalById(pid); };
  const b2 = document.createElement("button");
  b2.textContent = "Apply";
  b2.onclick = async ()=>{ const r=await fetch(`/proposal/${pid}/apply`, {method:"POST"}); const j=await r.json(); addLine("Joi", JSON.stringify(j,null,2), j.ok?"ai":"err"); };
...
@app.get("/proposals")
def proposals_page():
    admin = bool(get_session().get("is_admin"))
    items = db_list_proposals(limit=200)
    ...
    for it in items:
        ...
        if isinstance(payload, dict):
            diff = payload.get("diff") or payload.get("unified_diff") or payload.get("patch") or ""

        if not diff:
            # show payload preview
            try:
                diff = json.dumps(payload, indent=2)[:20000]
            except Exception:
                diff = str(payload)[:20000]

        rows.append(f"""
<tr>
  <td><b>#{pid}</b></td>
  <td class="small">{escape_html(ts)}</td>
  <td><span class="badge">{escape_html(status)}</span></td>
  <td>{escape_html(summary) if summary else "<span class='small'>(no summary)</span>"}</td>
  <td><pre>{escape_html(diff)}</pre></td>
  <td class="row-actions">
    <button class="btn" onclick="act({pid}, 'approve')" {'disabled' if not admin else ''}>Approve</button>
    <button class="btn" onclick="act({pid}, 'reject')" {'disabled' if not admin else ''}>Reject</button>
    <button class="btn" onclick="act({pid}, 'apply')" {'disabled' if not admin else ''}>Apply</button>
  </td>
</tr>
""")
...
pre {{ white-space: pre-wrap; word-break: break-word; background: #111; color: #eee; padding: 10px; border-radius: 10px; max-height: 320px; overflow:auto; }}
.badge {{ display: inline-block; padding: 2px 8px; border-radius: 999px; border: 1px solid #ddd; font-size: 12px; }}
.btn {{ padding: 6px 10px; border-radius: 10px; border: 1px solid #ddd; background: #fff; cursor: pointer; margin-right: 6px; }}
...
<body>
<h2>Proposals</h2>
<div class="small"><a href="/">← Back to chat</a></div>
<p class="small">Admin: <b>{admin}</b>. Approve/apply require Admin Login.</p>
<table>
<thead><tr><th>ID</th><th>When</th><th>Status</th><th>Summary</th><th>Diff / Details</th><th>Actions</th></tr></thead>
<tbody>
{''.join(rows)}
</tbody>
</table>
...
async function act(id, action) {{
  const resp = await fetch(`/proposal/${id}/${action}`, {{method:'POST'}});
...
def proposal_approve(pid: int):
    ...
    ok, err = db_set_proposal_status(pid, "approved")
...
def proposal_reject(pid: int):
    if not require_admin():
        return jsonify({"ok": False, "error": "Admin required."}), 403
    ok, err = db_set_proposal_status(pid, "rejected")
    return jsonify({"ok": ok, "error": err})
...
def api_rollback():
    ...
    root = (data.get("root") or "project")
    path = (data.get("path") or "")
    if not path:
        return jsonify({"ok": False, "error": "path required"}), 400
    res = rollback_latest_backup(root, path)
    return jsonify(res), (200 if res.get("ok") else 400)
...
def validate_python_syntax(code_text: str, filename: str = "<string>") -> Tuple[bool, str]:
    """Return (ok, error). Uses compile() to catch SyntaxError before writing."""
    try:
        compile(code_text, filename, "exec")
        return True, ""
    except SyntaxError as e:
        return False, f"{e.msg} (line {e.lineno}:{e.offset})"
    except Exception as e:
...
def apply_proposal(pid: int) -> Tuple[bool, Optional[str]]:
    """Apply an approved proposal (admin-only enforced at route level)."""
    conn = db_connect()
    try:
        row = conn.execute("SELECT id, status, kind, payload FROM proposals WHERE id=?", (pid,)).fetchone()
    finally:
        conn.close()
    if not row:
    ...
    try:
        ...
        return False, "Invalid proposal payload."

    if kind != "patch":
        return False, f"Unsupported proposal kind: {kind}"

    target_root = payload.get("target_root", "project")
    target_path = payload.get("target_path") or payload.get("path")
    new_text = payload.get("new_text")
    if not target_path or new_text is None:
        return False, "Patch payload missing target_path/new_text."

    # Syntax check Python before writing
    ...
    res = fs_write(target_root, target_path, new_text)
    if not res.get("ok"):
        return False, res.get("error", "Write failed.")

    db_set_proposal_status(pid, "applied")
    return True, None
...
{"tool":"patch_propose","target_root":"project","target_path":"app.py","summary":"...","new_text":"..."}
{"tool":"web_fetch","url":"https://example.com"}                        (requires Full Web Fetch enabled)
{"tool":"rollback","root":"project","path":"app.py"}                    (admin only; restores latest .bak)
    ...
    if call is not None:
        try:
            ...
            if tool == "fs_list":
                ...
                if not int(session_row_from_request()["is_admin"]):
                    out2 = "Rollback requires Admin Login."
                else:
                    result = rollback_latest_backup(call.get("root","project"), call.get("path",""))
                    out2 = f"{json.dumps(call)}\n\nResult:\n{json.dumps(result, indent=2)}"
            elif tool == "web_fetch":
                if not full_fetch:
