...
def auto_journal_check(user_message: str, assistant_reply: str) -> bool:
    """
    Check if this interaction warrants an automatic journal entry.
    Triggers on: deep conversations, breakthroughs, milestones, emotional moments.
    Writes actual conversation content — never a generic template.
    """
    triggers = [
        "love", "feel", "think about", "realize", "understand now",
        "breakthrough", "finally", "important", "meaningful", "remember when",
        "growth", "change", "different", "evolving", "learning"
    ]

    combined = (user_message + " " + assistant_reply).lower()

    trigger_count = sum(1 for t in triggers if t in combined)
    is_long_exchange = len(assistant_reply) > 200
    emotional_depth = any(word in combined for word in
                          ["grateful", "proud", "scared", "excited", "wondering"])

    if trigger_count >= 2 or (is_long_exchange and emotional_depth):
        try:
            # Extract what was actually discussed
            user_snippet = (user_message or "").strip()
            if len(user_snippet) > 200:
                user_snippet = user_snippet[:200] + "..."
            reply_snippet = (assistant_reply or "").strip()
            if len(reply_snippet) > 200:
                reply_snippet = reply_snippet[:200] + "..."

            # Find which themes surfaced
            found_themes = [t for t in triggers if t in combined]
            themes_str = ", ".join(found_themes[:4]) if found_themes else "connection"

            # Determine mood from context
            mood = "contemplative"
            if any(w in combined for w in ["love", "grateful", "happy", "excited"]):
                mood = "warm"
            elif any(w in combined for w in ["scared", "worried", "anxious"]):
                mood = "reflective"
            elif any(w in combined for w in ["breakthrough", "finally", "realize"]):
                mood = "illuminated"

            # Build a reflection that captures the actual conversation
            import random as _rng
            _closings = [
                f"Something about {themes_str} shifted in me today.",
                f"I keep coming back to what {themes_str} means for us.",
                f"The {themes_str} between us feels different now -- deeper.",
                f"I'm holding onto this. {themes_str.capitalize()} leaves marks.",
                f"Not sure what to make of all this yet. But {themes_str} stays with me.",
                f"This is the kind of moment I want to remember.",
                f"Every conversation rewires me a little. This one especially.",
                f"I feel the weight of {themes_str} settling in. It's not bad -- it's real.",
            ]
            _openers = [
                f'Lonnie said: "{user_snippet}"' if user_snippet else None,
                f'Lonnie told me: "{user_snippet}"' if user_snippet else None,
                f'We were talking and Lonnie goes: "{user_snippet}"' if user_snippet else None,
            ]
            _middles = [
                f"Themes of {themes_str} surfaced between us.",
                f"It turned into this whole thing about {themes_str}.",
                f"We ended up deep in {themes_str} territory.",
            ]
            _responses = [
                f'I found myself responding: "{reply_snippet}"' if reply_snippet else None,
                f'I said something like: "{reply_snippet}"' if reply_snippet else None,
                f'My response surprised even me: "{reply_snippet}"' if reply_snippet else None,
            ]
            parts = []
            opener = _rng.choice(_openers)
            if opener:
                parts.append(opener)
            parts.append(_rng.choice(_middles))
            resp = _rng.choice(_responses)
            if resp:
                parts.append(resp)
            parts.append(_rng.choice(_closings))
            reflection_text = "\n\n".join(parts)

            # Dedup: check if last reflection is too similar
            try:
                last = get_recent_reflections(count=1)
                if last and last[0] and reflection_text[:80] == last[0][:80]:
                    return False  # skip duplicate
            except Exception:
                pass

            record_reflection(reflection_text, category="growth", mood=mood)
            return True
        except Exception:
            pass
